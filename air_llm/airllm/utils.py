...
def split_and_save_layers(checkpoint_path, layer_shards_saving_path=None,
    ...
    layer_file_to_keys = {
        layer: {
            filename: [k for k in keys if index[k] == filename]
            for filename in sorted(set(index[k] for k in keys))
        }
    ...
    for file_to_keys in layer_file_to_keys.values():
        for filename in file_to_keys:
            file_refcount[filename] += 1

    last_loaded_bin = None
    last_bin_state_dict = None

    for i, layer in enumerate(tqdm(layers), start=1):
        file_to_keys = layer_file_to_keys[layer]
        layer_sd = {}

        for filename, needed_keys in file_to_keys.items():
            to_load = checkpoint_path / filename
            if not os.path.exists(to_load):
                if repo_id is None:
                    raise FileNotFoundError(f'Missing shard {to_load} and no repo_id provided for download fallback.')
        ...
        del layer_sd
        if (i % SPLIT_CLEANUP_INTERVAL == 0) or (i == len(layers)):
            clean_memory()
...
def find_or_create_local_splitted_path(model_local_path_or_repo_id, layer_shards_saving_path=None,
                                       ...
                                       delete_original=False, repo_id_for_missing=None):
    ...
    if os.path.exists(p):
        if os.path.exists(p / 'pytorch_model.bin.index.json') or os.path.exists(p / 'model.safetensors.index.json'):
            return p, split_and_save_layers(
                ...
                repo_id=repo_id_for_missing, hf_token=hf_token,
            )
